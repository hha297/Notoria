...
export const getWorkspaceActivitySnapshot = cache(
  async (workspaceId: string): Promise<WorkspaceActivitySnapshot> => {
    ...
    const [vocab, writing, theory, practice] = await Promise.all([
      db.select({ value: count() }).from(vocabularyWords).where(owned),
      db
        .select({ value: count() })
        .from(exercises)
        .where(
