...
type ExerciseHintProps = {
  ...
  onRevealAnswer?: () => void;
  /** When omitted, only the timed answer-reveal controls are shown (no hint text). */
  children?: ReactNode;
};
...
function ExerciseHintInner({
  ...
  const hasHintContent = Boolean(children);
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            <p className="text-sm text-ink sm:text-base" aria-live="polite">
              {hasHintContent
                ...
                : t("answerAvailableIn", { seconds: secondsLeft })}
            </p>
            <Button
              type="button"
              variant="ghost"
            ...
            >
              {hasHintContent ? t("showNow") : t("showAnswerNow")}
            </Button>
          </div>
        )}
        ...
        {canRevealAnswer && correctAnswer && onRevealAnswer ? (
          <div
            className={cn(
              ...
              hasHintContent && "border-t border-hairline-cloud pt-3",
            )}
          >
